...
<%@ include file="ProName.jsp" %>
 ...
 try {
	 ...
	 System.out.println("Benadryl Connects to DB");
	
	 String benadryl = "SELECT D.DID, D.GenericName, D.Description, D.Form FROM Drug D WHERE D.BrandName LIKE '%Benadryl%' ";
	 PreparedStatement ps = con.prepareStatement(benadryl);
	 ResultSet res = ps.executeQuery();
	 out.println("<div style=margin-top:100px>");
	 out.println("<h2 style='color:white; margin-left:500px'>Results for Searching Benadryl Product Name</h2>");
	 ...
	 while(res.next()){
		 ...
		 String benStore = "SELECT St.name, St.street, St.city, St.zip_code,St.phone_number FROM Store St  WHERE St.SID IN(SELECT SB.SID FROM SoldByStore SB WHERE SB.DID = '"+ id +"')  ";
		 PreparedStatement ps1 = con.prepareStatement(benStore);

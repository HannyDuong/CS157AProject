...
<%@ include file="ProName.jsp" %>
 ...
 try {
	 ...
	 System.out.println("Pepcid Connects to DB");
	
	 String pepid = "SELECT D.DID, D.GenericName, D.Description, D.Form FROM Drug D WHERE D.BrandName LIKE '%Pepcid%' ";
	 PreparedStatement ps = con.prepareStatement(pepid);
	 ResultSet res = ps.executeQuery();
	 out.println("<div style=margin-top:100px>");
	 out.println("<h2 style='color:white; margin-left:500px'>Results for Searching Pepcid Product Name</h2>");
	 ...
	 while(res.next()){
		 ...
		 String pepStore = "SELECT St.name, St.street, St.city, St.zip_code,St.phone_number FROM Store St  WHERE St.SID IN(SELECT SB.SID FROM SoldByStore SB WHERE SB.DID = '"+ id +"')  ";
		 PreparedStatement ps1 = con.prepareStatement(pepStore);

...
<title>Category Cough And Cold</title>
</head>
<body>
<%@ include file="Category.jsp" %>
 ...
 try 
 {
	 ...
	 System.out.println("CategoryCoughAndCold Connects to DB");
	
	 String CategoryCoughAndCold = "SELECT D.DID, D.GenericName, D.BrandName, D.Form, D.Description, D.Notes FROM Drug D WHERE D.Category = 'CoughAndCold'";
	 PreparedStatement ps = con.prepareStatement(CategoryCoughAndCold);
	 ResultSet res = ps.executeQuery();
	 
	 out.println("<div style=margin-top:100px>");
	 out.println("<h2 style='color:white; margin-left:400px'>Results for Searching Cough and Cold Category</h2>");
	 ...
	 while(res.next())
	 {
		 ...
		 String StoreC = "SELECT St.name, St.street, St.city, St.zip_code, St.phone_number FROM Store St WHERE St.SID IN(SELECT SB.SID FROM SoldByStore SB WHERE SB.DID = '"+ col1 +"')";
		 PreparedStatement ps1 = con.prepareStatement(StoreC);
